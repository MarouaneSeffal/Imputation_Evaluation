...
#'
#' @param original A data frame containing original values with columns "unit_id", "iter", and variables.
#' @param imputed A data frame containing imputed values with the same structure as original.
...
#'
#' @examples
#' original <- data.frame(unit_id = c(1,2,3,4), iter = c(1,1,1,1), variable_a = c(10,20,30,40), variable_b = c(5,15,25,35))
#' imputed <- data.frame(unit_id = c(1,2,3,4,5), iter = c(1,1,1,1,1), variable_a = c(10,25,30,45,50), variable_b = c(5,15,20,35,40))
#' detect_changes(original, imputed, "unit_id")


out_imp_create <- function(data, imputed, unit_id) {

  # Ensure one iteration exists on imputed each run
  if (length(unique(imputed$iter)) > 1) {
    stop("out_imp creation cannot be performed since Imputed data.frame contains more than one iteration.
  Please ensure a unique iteration exists before running 'out_imp_create'.")
  }
  ...
  original <- merge(unit_id_imp, data, by = unit_id, all.x = TRUE)
